    ...
    <tbody>
        <%
            try {
                ...
                while (rset.next()) {
                    ResultSet rset2 = con.consulta("select i.F_Cant from tb_medica m, tb_invenuni i where m.F_ClaPro = i.F_ClaPro and i.F_ClaCli = '" + request.getParameter("F_ClaCli") + "' and F_FecInv = '" + request.getParameter("F_FecInv") + "' and i.F_ClaPro = '" + rset.getString("F_ClaPro") + "'");
                    int cantInven = 0;
                    while (rset2.next()) {
                        cantInven = rset2.getInt("F_Cant");
                    }
                    int dif = rset.getInt("F_Cant") - cantInven;
        ...
        <tr>
            ...
            <td class="text-right"><%=formatter.format(cantInven)%></td>
            <td class="text-right"><%=formatter.format(dif)%></td>
        </tr>
        <%
                }
                con.cierraConexion();
