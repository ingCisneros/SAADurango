 ...
 * @author Sistemas
 */
public class FileUploadDoctores extends HttpServlet {

    LeerDoctores lee = new LeerDoctores();
    CargaDoctores carga = new CargaDoctores();
            ...
            try {
                ...
                while (itr.hasNext()) {
                    ...
                    if (item.isFormField()) {
                        ...
                        if (FileUploadDoct.processFile(path, item)) {
                            //response.getWriter().println("file uploaded successfully");
                            sesion.setAttribute("ban", "1");
                            if (lee.obtieneArchivo(path, item.getName())) {
                                String mensaje = carga.cargaDoct((String) sesion.getAttribute("id_usu"), item.getName());
                                out.println("<script>alert('"+mensaje+"')</script>");
                                out.println("<script>window.location='CatalogoIsem/Doctores.jsp'</script>");
                            }
