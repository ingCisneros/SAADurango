 ...
 * @author Sistemas
 */
public class FileUploadJurisdiccion extends HttpServlet {

    LeerJurisdiccion lee = new LeerJurisdiccion();
    CargaJurisdiccion carga = new CargaJurisdiccion();
            ...
            try {
                ...
                while (itr.hasNext()) {
                    ...
                    if (item.isFormField()) {
                        ...
                        if (FileUploadJuris.processFile(path, item)) {
                            //response.getWriter().println("file uploaded successfully");
                            sesion.setAttribute("ban", "1");
                            if (lee.obtieneArchivo(path, item.getName())) {
                                String mensaje = carga.cargaJuris((String) sesion.getAttribute("id_usu"), item.getName());
                                out.println("<script>alert('"+mensaje+"')</script>");
                                out.println("<script>window.location='CatalogoIsem/Jurisdicciones.jsp'</script>");
                            }
