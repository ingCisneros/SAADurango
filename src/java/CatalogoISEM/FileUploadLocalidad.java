 ...
 * @author Sistemas
 */
public class FileUploadLocalidad extends HttpServlet {

    LeerLocalidad lee = new LeerLocalidad();
    CargaLocalidad carga = new CargaLocalidad();
            ...
            try {
                ...
                while (itr.hasNext()) {
                    ...
                    if (item.isFormField()) {
                        ...
                        if (FileUploadLocal.processFile(path, item)) {
                            //response.getWriter().println("file uploaded successfully");
                            sesion.setAttribute("ban", "1");
                            if (lee.obtieneArchivo(path, item.getName())) {
                                String mensaje = carga.cargaLocali((String) sesion.getAttribute("id_usu"), item.getName());
                                out.println("<script>alert('"+mensaje+"')</script>");
                                out.println("<script>window.location='CatalogoIsem/Localidades.jsp'</script>");
                            }
