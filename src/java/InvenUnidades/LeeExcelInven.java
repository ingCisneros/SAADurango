 ...
 */
public class LeeExcelInven {

    private Vector vectorDataExcelXLSX = new Vector();
    ConectionDB con = new ConectionDB();

    public boolean obtieneArchivo(String path, String file) {
        String excelXLSXFileName = path + "/inventarios/" + file;
    ...
    public void displayDataExcelXLSX(Vector vectorData) {
        ...
        for (int i = 0; i < vectorData.size(); i++) {
            ...
            String Fecha = "";
            try {
                Fecha = df2.format(df.parse(vectorCellEachRowData.get(2).toString()));
            } catch (ParseException e) {
            }
            String Cant = ((int) Double.parseDouble(vectorCellEachRowData.get(3).toString()) + "");
            //qry = qry + "'" + Clave.trim() + "' ";

            String qry = "insert into tb_invenuni values ('" + F_ClaCli + "','" + agrega(ClaPro) + "','" + Fecha.trim() + "','" + Cant.trim() + "',0)";
            //String qryUpdate = "update tb_invenuni set F_Cant = '" + Cant.trim() + "' where F_ClaPro = '" + agrega(ClaPro) + "' and F_ClaCli = '" + F_ClaCli + "'";
