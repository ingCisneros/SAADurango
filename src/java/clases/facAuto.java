...
public class facAuto {
    ...
    public void requerimiento(String claveUni) {
        ...
        try {
            ...
            String consu = "SELECT F_IdUni,F_IdCla,SUM(F_Cajas),SUM(F_Pz),F_Pres,F_Sts FROM tb_invTemp WHERE F_IdUni='" + claveUni + "'  GROUP BY F_IdCla  ORDER BY F_IdCla+0 ";

            ResultSet rc = con.consulta(consu);

            while (rc.next()) {
                invUni inv = new invUni();
                inv.setCajas(rc.getInt("SUM(F_Cajas)"));
                inv.setClaMed(rc.getString("F_IdCla"));
                inv.setClaUNi(rc.getInt("F_IdUni"));
                inv.setPres(rc.getInt("F_Pres"));
                inv.setPz(rc.getInt("SUM(F_Pz)"));
                inv.setStatus(rc.getInt("F_Sts"));
                con.insertar("INSERT INTO tb_invUniAte (F_IdUni,F_IdCla,F_Cajas,F_Pz,F_Pres,F_Sts) VALUES ('"+inv.getClaUNi()+"','"+inv.getClaMed()+"','"+inv.getCajas()+"','"+inv.getPz()+"','"+inv.getPres()+"','"+inv.getStatus()+"');");
                
            }

            String qry = "SELECT F_IdUni,F_IdCla,SUM(F_Cajas),SUM(F_Pz),F_Pres,F_Sts FROM tb_invTemp WHERE F_IdUni='" + claveUni + "'  GROUP BY F_IdCla  ORDER BY F_IdCla+0 ";
            int cajas = 0;
            ResultSet rs = con.consulta(qry);

            while (rs.next()) {
                invUni inv = new invUni();
                inv.setCajas(rs.getInt("SUM(F_Cajas)"));
                inv.setClaMed(rs.getString("F_IdCla"));
                inv.setClaUNi(rs.getInt("F_IdUni"));
                inv.setPres(rs.getInt("F_Pres"));
                inv.setPz(rs.getInt("SUM(F_Pz)"));
    ...
    public void Rural(String claveUni)
    {
         ...
         try {
            ...
            while (rs.next()) {
               ...
               String insert = "INSERT INTO tb_unireq (F_ClaUni,F_ClaPro,F_CajasReq,F_Status) VALUES ('" + inv.getClaUNi() + "','" + inv.getClaMed() + "'," + inv.getCajas() + ",0)";
               con.insertar(insert);
