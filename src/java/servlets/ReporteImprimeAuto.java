...
public class ReporteImprimeAuto extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if ((Fecha1 != "") && (Fecha2 != "")){
                FoliosFactura = con.consulta("SELECT F.F_ClaDoc FROM tb_factura F WHERE F.F_FecEnt between '"+Fecha1+"' and '"+Fecha2+"' GROUP BY F.F_ClaDoc ORDER BY F.F_ClaDoc+0");
            ...
            if(F_FacGNKLAgr.equals("")){
                ...
                if(F_Contrato.equals("")){
                    ...
                    F_FecIni = F_Dia2+"/"+F_Mes+"/"+F_Ano;
                    F_FecFin = F_Dia1+"/"+F_Mes+"/"+F_Ano;
                    ...
                    for (int Reporte = 1; Reporte<=8; Reporte++){
                            ...
                            out.println(" <script>window.open('ReportesPuntos/ReporteVerticalImp.jsp?User="+sesion.getAttribute("nombre")+"', '', 'width=1200,height=800,left=50,top=50,toolbar=no'); </script>");
                            con.actualizar("INSERT INTO tb_imprepvalauto VALUES (0,'"+F_DesSur+"','"+F_Descob+"','"+F_DesSum+"','ABIERTO','"+PzsReq+"','"+PzsSur+"','"+CosMed+"','"+F_CosServ+"','"+(CosMed+F_CosServ)+"','"+IVA+"','"+(CosMed+F_CosServ+IVA)+"','"+PzsNOSur+"','"+CosNOSur+"','"+F_DesUni+"','"+F_DesJur+"','"+F_Region+"','"+F_DesMun1+"','"+F_DesLoc+"','"+F_FolCon+"','"+F_FecIni+"','"+F_FecFin+"','"+F_SecIni+"','"+F_SecFin+"','"+F_Cvepro+"','"+F_Contrato+"','"+F_FacGNKLAgr+"','"+sesion.getAttribute("nombre")+"',curdate())");
                    ...
                    out.println(" <script>window.open('ReportesPuntos/ReporteConcentradoImp.jsp?User="+sesion.getAttribute("nombre")+"', '', 'width=1200,height=800,left=50,top=50,toolbar=no'); </script>");
                            F_SecIniC = 0;
                            F_SecFinC = 0;
                    ...
                    for (int Reporteq = 1; Reporteq<=8; Reporteq++){
                           ...
                           // con.actualizar("delete from tb_imprepval");
                            ...
                            out.println(" <script>window.open('ReportesPuntos/ReporteHorizontalImp.jsp?Use="+sesion.getAttribute("nombre")+"', '', 'width=1200,height=800,left=50,top=50,toolbar=no'); </script>");
                    ...
                    out.println("<script>window.print('ReportesPuntos/ReporteHorizontalImp.jsp)</script>"); 
                    out.println("<script>window.history.back()</script>");
            }
            }
            }else{                
